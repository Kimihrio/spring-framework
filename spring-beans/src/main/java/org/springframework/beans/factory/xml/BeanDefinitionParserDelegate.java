...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			...
			//将获取到的信息封装进 BeanDefinitionHolder
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			//创建用于承载属性的 AbstractBeanDefinition类型的 GenericBeanDefinition（BeanDefinition）
			...
			//解析元数据（需要使用元数据的时候可以通过 BeanDefinition 的 getAttribute(key) 方法进行获取）
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
		...
		//解析 scope 属性
			...
			//在嵌入 BeanDefinition 情况下则使用父类默认的属性
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) {
				...
				//用 key、value 构造 BeanMetadataAttribute
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//仅当在 Spring 默认 bean的子元素下且为 <lookup-method 时有效
	...
	public void parseReplacedMethodSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//仅当在 Spring 默认 bean的子元素下且为 <replaced-method 时有效
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						//ConstructorArgumentValues.ValueHolder 封装解析出来的元素
						...
						if (bd.getConstructorArgumentValues().hasIndexedArgumentValue(index)) {
							...
							//添加至 BeanDefinition 的 constructorArgumentValues 的 indexedArgumentValue 属性中
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		...
		//根据命名空间找到对应的 NamespaceHandler（初始化时已经初始化成 DefaultNamespaceHandlerResolver）
